    ...
    <div class="tin-right on">
        <div class="testList">
            <el-row class="theory">
                <div class="theory-list" style="padding-bottom:0">
                    ...
                    <a href="javascript:void(0);" class="delay"  @click="delayClick(getTalent.id,getTalent.uid,getTalent.pid)">
                        <img :src="reUrl" alt="">未通过
                    </a>
                    <div class="testList-hdline1"></div>
                    </div>
                    <div class="testList-bd">
                    <div style="height:0;"><div style="" ref="twidth"></div></div>
                    <el-row class="tscore" v-if="detail.skills && detail.skills.length > 0">
                        ...
                        <el-tab-pane :label="getcommonFun.tabKey(item.pid)" :name="String(detail.skills.length)" v-for="(item,index) in detail.skills" :key="index">
                            ...
                            <el-col :span="12" class="green">
                                <template v-if="getTalent.skill !== null">
                                <el-col :span="12">
                                    <el-col><span>{{getTalent.skill.score}}</span>分</el-col>
                                </el-col>
                                <el-col :span="12">
                            ...
                            <el-col :span="12" class="green">
                                <template>
                                <el-col v-if="getTalent.skillOperation !== null">
                                    <el-col :span="12" :class="getTalent.skillOperation.status == 2?'green':'brown'">
                                    <el-col><span>{{getTalent.skillOperation.score}}</span>分</el-col>
                                    </el-col>
                                    <el-col :span="12">
                                    <el-button class="look" @click="lookDg(item.pid,item.wid,1)">查看</el-button>
                                    </el-col>
                                </el-col>
                                <el-col :span="12" class="brown" v-else>--</el-col>
                                </template>
                            </el-col>
                            </el-row>
                            <el-row>
                            ...
                            <el-col :span="12" class="green">
                                <el-col>
                                <el-col v-if="getTalent.record !== null"><span>{{getTalent.record.score}}</span>分</el-col>
                                <el-col v-else>--</el-col>
                                </el-col>
                            </el-col>
                            </el-row>
                            <el-row>
                            <el-col :span="12" class="bold1" v-if="getTalent.record !== null">综合测试分</el-col>
                            <el-col :span="12" class="bold1" v-else></el-col>
                            <el-col :span="12" class="red">
                                <el-col v-if="getTalent.record !== null">{{totalScore?totalScore+'分':'--'}}</el-col>
                            </el-col>
                            </el-row>
                            <el-row class="tping">
                            <el-row class="tping-hd" style="margin-bottom:0" v-if="getTalent.record !== null">
                                <el-col :span="8">岗位级别：</el-col>
                                <el-col :span="16">
                                <template v-if="detail.level && detail.level.length > 0">
                                <el-row v-for="(sitem,index) in detail.leve" :key="index">
                    ...
                    <div class="p15" v-if="getTalent.record">
                        ...
                        <el-row class="tb1" style="border-bottom:0">
                            ...
                            <el-col :span="12" style="padding-right:10px;">
                                ...
                                <template v-for="(item,index) in photos">
                                    ...
                                    <template v-for="(sitem,index) in item.pic">
                                        <img :src="sitem.path | imgFormat" alt="" class="user-img" :key="index+'pic1'">
                                    </template>
                                    </el-row>
                            ...
                            <el-col :span="12">
                                ...
                                <el-row>
                                <el-col class="gray">籍贯：<span class="blak" style="margin-left:2px">{{detail.nativePlace !== null?this.getcommonFun.province(detail.nativePlace):'--'}}</span></el-col>
                                </el-row>
                                <el-row>
                        ...
                        <el-collapse v-model="activeNames">
                            ...
                            <el-row class="tb1" style="border-bottom:0">
                                <el-row class="healthImgList">
                                    <template v-if="photos && photos.length > 0">
                                    <span v-for="(item,index) in photos" :key="index">
                            ...
                            <el-row class="tb1" style="border-bottom:0">
                                ...
                                <el-col class="gray">个人特长：
                                    <span class="blak">{{speciality && speciality.length !== 0?getcommonFun.tabKey(speciality+''):'--'}}</span>
                                </el-col>
                                </el-row>
                                <el-row>
                                <el-col class="gray">可工作区域：
                                    <span class="blak">{{detail.workPlace && detail.workPlace.length !== 0?getcommonFun.tabKey(detail.workPlace+''):'--'}}</span>
                                </el-col>
                                </el-row>
                                <el-row>
                            ...
                            <el-row class="tb1" style="border-bottom:0">
                                ...
                                个人标签：
                                <span class="blak">
                                    {{detail.style?getcommonFun.tabKey(detail.style+''):'--'}}
                                    {{detail.personality?'性格'+getcommonFun.tabKey(detail.personality+''):'--'}}
                                </span>
                                </el-row>
                                <template v-if="detail.assessment !== null">
                                <el-row class="zwpj" v-if="instr == 0">
                                    ...
                                    <div class="video-img" v-show="vdshow" style=""></div>
        ...
        <el-dialog :title="'专业技能'+`${getcommonFun.tabKey(keyName)}`+'测试'" :visible.sync="testDialogVisible" :close-on-click-modal="false" width="30%" center class="addDialog active on">
            ...
            <el-row class="ltest" :class="cur1 == item.id?'on':''" v-for="item in ltest" :key="item.id">
                ...
                <span class="brown" v-if="item.status !== 1">(未通过)</span>
                </el-row>
            ...
            <el-row class="ltest-close" style="display:none;">
                <el-button @click="testDialogVisible = false">取消</el-button>
                <el-button v-if="reTest == '确认'" @click="testDialogVisible = false">确认</el-button>
                <el-button v-if="reTest == '重新测试'" @click="reTestDialog(getTalent.id,getTalent.pid,getTalent.uid)">重新测试</el-button>
            </el-row>
            </el-row>
            </el-row>
        </el-dialog>
...
<script>
import { lookAnswer,reAnswer } from '@/api/api'
import { mapGetters } from 'vuex'
export default {
    name:'',
    data () {
        return {
            reUrl: require('@/assets/images/reback.png'),
