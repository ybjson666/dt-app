<template>
  <div class="pages-container">
      ...
      <div class="page-table-content">
        ...
        <div class="tablent-content">
          <el-table :data="tableDataList" highlight-current-row border style="width: 100%" @row-click="cellClick">
            <el-table-column prop="orderNum" label="订单号" width="">
              <template slot-scope="scope">
                <el-popover trigger="hover" placement="bottom">
                  <div class="emname color1">{{scope.row.orderDemandCommon.id}}</div>
                  <span class="ellips" slot="reference" style="display:block;width:100%;">
                    {{scope.row.orderDemandCommon.id}}
                  </span>
                </el-popover>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="servicer" label="服务人员" width="">
              <template slot-scope="scope">
            ...
            <el-table-column prop="serviceTime" label="服务时间" width="200px">
              <template slot-scope="scope">
                <p class="pays tagnlf" style="padding-left:.2rem;"><i class="gn1"></i>{{scope.row.orderDemandCommon.startTime}}</p>
                <p class="pays tagnlf" style="padding-left:.2rem;"><i class="gn2"></i>{{scope.row.orderDemandCommon.endTime == null?'长期':scope.row.orderDemandCommon.endTime}}</p>
              </template>
            </el-table-column>
            <el-table-column prop="follow" label="跟进人" >
              <template slot-scope="scope">
                <span class="color1">{{scope.row.orderDemandCommon.documentName||'--'}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="followStatus" label="跟进状态" width="120px">
              <template slot-scope="scope">
                <span class="theme2" v-if="scope.row.followStatus == 9">已完成</span>
              </template>
            </el-table-column>
    ...
    <!-- 雇主详情弹窗 -->
    <OrderProcessDetail ref="orderDetail"  :row="curUser" />
    <!-- 聊天 -->
...
<script>
import { demandList,employee } from '@/api/api'
    ...
    methods: {
      getDemandList(){ // 历史订单列表
        let params = {
          ...
          oid:this.oid,
          startCreaTime:this.startCreaTime,
          endCreaTime:this.endCreaTime 
        }
      ...
      changeDate(val) {
        ...
        if(val){
          ...
          this.startCreaTime= `${startStr} 00:00:00`
          this.endCreaTime=`${endStr} 00:00:00`
        }else{
          this.startCreaTime=""
          this.endCreaTime=""
        }
