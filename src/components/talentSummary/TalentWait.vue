<template>
  <div class="pages-container">
          ...
          <el-table :data="tableDataList" highlight-current-row @row-click="cellClick" border style="width: 100%">
            ...
            <el-table-column prop="status" label="状态" width="">
              <template slot-scope="scope">
                ...
                <span class="dsh" v-if="scope.row.status == 2">等单中</span>
                <span class="dsh" v-else-if="scope.row.status == 3">上单中</span>
                 <span class="dsh" v-else-if="scope.row.status == 4">暂停中</span>
              </template>
            </el-table-column>
    ...
    <el-drawer title="人才考核详情" :visible.sync="drawer" :direction="direction">
      <TalentInfo :detail="detail" :jiedan="jiedan" @updateJiedan="updateJiedan" @updateList="getList" @close="closeDialog"/>
    </el-drawer>
...
<script>
import { summaryList,getDetail,match } from '@/api/api'
    ...
    methods: {
      ...
      getList(){//获取表格数据
        let params = {
          ...
          status: 2, //上单状态 2可上单 3上单中 4暂停中 5黑名单 11我的收藏
      ...
      matchEmplee(oid){//从订单详情智能匹配跳过来后再进行匹配
         let params = {
          app: 3,
          device: 5,
          oid,
