          ...
          <el-table :data="tableDataList" highlight-current-row  @row-click="cellClick" border style="width: 100%">
            ...
            <el-table-column prop="status" label="状态" width="">
              <template slot-scope="scope">
                ...
                <span class="onduty" v-if="scope.row.status == 3">已上岗</span>
                <span class="suspend" v-if="scope.row.status == 4">暂停中</span>
                <!-- <span class="blacklist" v-if="scope.row.status == '黑名单'">{{scope.row.status}}</span> -->
              </template>
            </el-table-column>
